...
function history(seed: string, solRaised: number, points = 48): number[] {
  const rnd = mulberry32(hashString(seed));
  const end = spotPrice(solRaised);
  const start = spotPrice(Math.max(0.2, solRaised * (0.15 + rnd() * 0.5)));
  const out: number[] = [];
  let v = start;
...
export const coins: Coin[] = seeds.map(({ launcherSeed, ...s }) => ({
  ...
  history: history(s.id, s.solRaised),
}));
...
export const siteStats = {
  paidToPeopleSol: coins.reduce((a, c) => a + c.personFeesSol, 0),
  coinsLaunched: 1_842,
  volume24hSol: coins.reduce((a, c) => a + c.volume24hSol, 0) * 4.7,
  peopleClaimed: 611,
};
